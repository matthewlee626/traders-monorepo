...
def add_user():
    ...
    if game is None:
        return Response('Game does not exist with game_id: ' + str(game_id), status=500)
    if game.status == 2:
        return Response('Game with this game_id ' + str(game_id) + ' has ended', status=500)
    game_users = game.players
    for user in game_users:
        if user == username:
            if game.status == 1:
                recent_transactions = game.trade_log
                if recent_transactions:
                    # if len(recent_transactions) > 5:
                    #     recent_transactions = recent_transactions[len(recent_transactions) - 5:]
                    recent_transactions = recent_transactions[::-1]
                elif not recent_transactions:
                    recent_transactions = []
                res = {
                'players':  [key for key, value in game_users.items()],
                ...
                "playerCard": game.get_players_cards().get(username),
                "revealedCards": game.get_revealed_cards(),
                "spread": game.get_current_spread_rem(),
                "recentTransactions": recent_transactions,
                "positions":game.get_all_players_position()
                }
            else:    
                res = {
                    'players':  [key for key, value in game_users.items()],
    ...
    if game.status == 1:
        return Response("The game has already started", status=400)
    # Check user limit
    if len(game_users) >= game.original_num_players:
...
def make_market():
    ...
    if game.status != 1:
        return Response('Game with game_id: ' + game_id + ' is not in progress.', status=401)
    try:
        data = request.json
        user_data = data['user']
        bid = int(data['bid'])
    ...
    return Response(json.dumps(resp_payload), status=200)

@gameB_api.route("/resetMarket", methods=['POST'])
def reset_market():
    '''
    {
        "password": password,
...
def transact():
    ...
    positions = game.get_all_players_position()
    recent_transactions = game.trade_log
    revealed_cards = game.get_revealed_cards()
    if recent_transactions:
        # if len(recent_transactions) > 5:
        #     recent_transactions = recent_transactions[len(recent_transactions) - 5:]
        recent_transactions = recent_transactions[::-1]
...
def drop():
    ...
    if password != game.password:
        return Response('Incorrect password for game with game_id: ' + game_id, status=401)
    if game.status == 2 or game.status==1:
        return Response('Game with game_id: ' + game_id + ' has already ended/in progress.', status=401)
    game_users = game.players
    ...
    cache.set(game_id, game)
    if game.status == 0:
        return Response("Drop Success", status=200)
    current_player = game.get_current_player()
    spread = game.get_current_spread_rem()
    positions = game.get_all_players_position()
    resp_payload = {
...
@gameB_api.route('/penalty', methods=['POST'])
def penalty():
    """
...
def get_player_positions():
    ...
    if password != game.password:
        return Response('Incorrect password for game with game_id: ' + game_id, status=401)
    if game.status == 2 or game.status == 0:
        return Response('Game with game_id: ' + game_id + ' is not in progress.', status=401)
...
def end_game():
    ...
    resp_payload = {
        "logs": game.trade_log,
        "truePnl": game.get_true_pnl(),
        "penalties": game.get_true_penalty(),
        "trueSum":game.get_true_sum(),
        "current_player": ''
    }
    return Response(json.dumps(resp_payload), status=200)
...
def get_turn_list():
    ...
    for i in range(len(transact_lst)):
        ...
        if len(cur_transact) == 2:
            for i in range(2):
                response = format_tranction_list(response, cur_transact[i], game_id)
        else: 
            response = format_tranction_list(response, cur_transact,game_id)
    response.reverse()
    return Response(json.dumps(response), status=200)
...
def get_players():
    ...
    return Response(', '.join(response), status=200)
...
@gameB_api.route("/myCard", methods=['POST'])
def my_card():
    """
    Shows what card the current player has
    ...
    if username not in cards:
        return Response('Username not found in game with game_id: ' + game_id, status=500)
    print("My card", cards)
    print(cards[username])
    resp = {
...
def start_game():
    ...
    current_player = game.get_current_player()
    resp_payload = {
        'msg': 'Game with game_id: ' + game_id + ' has started.',
        'current_player': current_player,
        'player_cards': game.get_players_cards()
    }
    return Response(json.dumps(resp_payload), status=200)
