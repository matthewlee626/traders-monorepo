...
class Game():
    def __init__(self, num_players=2, min_spread=1, max_spread=5, card_range_low=1, card_range_high=10, password=""):
        ...
        self.true_pnl = OrderedDict()
        self.true_penalty = OrderedDict()
        self.recent_transact_set = set()
        ...
        self.last_bid_lots_rem = None
        self.last_ask = None
        self.last_ask_lots = None
        self.last_ask_lots_rem = None
        self.last_market_maker = None
    ...
    def make_market(self, player, bid, bid_lots, ask, ask_lots):
        ...
        if player.player_id in self.recent_transact_set:
            self.new_market = True
        self.recent_transact_set = set()
        ...
        self.last_bid_lots_rem = bid_lots
        self.last_ask_lots = ask_lots
        self.last_ask_lots_rem = ask_lots
        # self.last_market_maker = player.player_id
        # Add to trade log
        self.trade_log.append({'player': player.player_id,
                                        'action': 'made market',
                                        'bid': bid,
                                        'bid_lots': bid_lots,
                                        'ask': ask,
    ...
    def resetMarket(self):
        self.last_bid *= -1
        self.last_ask *= -1
        self.last_bid_lots *= -1
        self.last_bid_lots_rem *= -1
        self.last_ask_lots *= -1
        self.last_ask_lots_rem *= -1
        # If last person in round, create a new set of rotation
        print("Market", self.roundTurn, self.turn, self.num_players, list(self.players.keys()))
        if self.roundTurn == self.num_players - 1:
            self.shuffle_round_order(None)
        # Increment each turn
        self.turn = self.turn + 1
        self.roundTurn = (self.roundTurn + 1) % self.num_players
        print("Updated turns:", self.turn, self.roundTurn)
    ...
    def add_user(self, player):
        if self.status != 0:
            return
        if len(self.players) >= self.original_num_players:
            return
        self.players[player] = None # initialize the player within the dict, but the value will be None.
        self.true_pnl[player] = 0
        self.true_penalty[player] = 0
    ...
    def transact(self, player, position, quantity):
        ...
        # err = self.check_turn(player)
        self.recent_transact_set.add(player.player_id)
        err = "You can't transact on your own market" if self.last_market_maker and self.last_market_maker.player_id == player.player_id else None
        if err != None:
            raise Exception(err)
        ...
        price = 0
        if position == 'BUY':
            if quantity > self.last_ask_lots_rem:
                raise Exception("{} total available lots. Enter a value less than or equal to that.".format(self.last_ask_lots_rem))
            ...
            self.last_ask_lots_rem -= quantity

        # If the player clicks on the sell button
        elif position == 'SELL':
            if quantity > self.last_bid_lots_rem:
                raise Exception(
                    "{} total available lots. Enter a value less than or equal to that.".format(self.last_bid_lots_rem))
            ...
            self.last_bid_lots_rem -= quantity
    ...
    def drop(self, player):
        ...
        if self.status == 0:
            if player in self.players:
                ...
                self.true_pnl.pop(player)
                self.true_penalty.pop(player)
                self.num_players -= 1
                return "DROP SUCCESS"
    ...
    def set_true_pnl(self):
        ...
        for k in self.players:
            ...
            self.true_pnl[player.player_id] += true_pnl
    
    def penalty(self):
        curr = self.get_current_player()
        self.true_penalty[curr] -= 10
        if self.roundTurn == self.num_players - 1:
            self.shuffle_round_order(curr)
        # Increment each turn
        self.turn = self.turn + 1
        self.roundTurn = (self.roundTurn + 1) % self.num_players
        new_curr_player = self.get_current_player()
        # print("Player", new_curr_player, self.last_market_maker.player_id)
        if self.last_market_maker and new_curr_player == self.last_market_maker.player_id:
            self.last_bid = None
            self.last_bid_lots = None
            self.last_bid_lots_rem = None
            self.last_ask = None
    ...
    def get_current_market_rem(self):
        """
        Gets current market set by the last player.

        :return: (list) [Bid, Bid Size, Ask, Ask Size]
        """
        return [self.last_bid, self.last_bid_lots_rem, self.last_ask, self.last_ask_lots_rem]
    ...
    def get_current_spread_rem(self):
        """
        Gets the most recent spread. 

        :return: (list) [(bid, bid_volume), (ask, ask_volume)]
        """
        spread = {"bid":{"price":self.last_bid, "volume": self.last_bid_lots_rem}, "ask": {"price": self.last_ask, "volume": self.last_ask_lots_rem}}
        return spread
    ...
    def check_transact_conditions(self):
        ...
        """
        err = None
        if self.turn == 0 and self.last_market_maker == None:
